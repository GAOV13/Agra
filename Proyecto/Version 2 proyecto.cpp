...
#define mpE map<tuple<int, int, int>, vector<Estado> >
#define endl '\n'
...
int dikstra(vvp &grafo, vi &info, int h){
    ...
    Estado e(1, 0, 0, 0);
    cola.push(e);
    mapa[make_tuple(1, 0, 0)].push_back(e);
    while(!cola.empty()){
        e = cola.top(); cola.pop();
        ...
        if(mapa[make_tuple(id_ciudad, id_hoteles, id_sinDesc)][0].dist == id_dist){
            if(id_sinDesc != 0 && id_hoteles + 1 <= h && info[id_ciudad] == 1){
                if(!mapa[make_tuple(id_ciudad, id_hoteles + 1, 0)].size()){
                    Estado e1(id_ciudad, id_dist, 0, id_hoteles + 1);
                    mapa[make_tuple(id_ciudad, id_hoteles + 1, 0)].push_back(e1);
                    //printf("%d %d %d %d %d\n", e1.ciudad, e1.hoteles, e1.dist, e1.sin_desc, -2);
                    cola.push(e1);
                }
                else{
                    Estado *e1 = &mapa[make_tuple(id_ciudad, id_hoteles + 1, 0)][0];
                    if(id_dist < (*e1).dist){
                        (*e1).dist = id_dist;
                        (*e1).sin_desc = 0;
                        Estado e2 = *e1;
            ...
            for(int i = 0; i < grafo[id_ciudad].size(); ++i){
                ...
                temp += id_dist;
                if(temp_desc <= 600){
                    if(!mapa[make_tuple(nueva_ciudad, id_hoteles, temp_desc)].size()){
                        Estado e1(nueva_ciudad, temp, temp_desc, id_hoteles);
                        mapa[make_tuple(nueva_ciudad, id_hoteles, temp_desc)].push_back(e1);
                        //printf("%d %d %d %d %d\n", e1.ciudad, e1.hoteles, e1.dist, e1.sin_desc, -4);
                        cola.push(e1);
                    }
                                        
                    else{
                        Estado *e1 = &mapa[make_tuple(nueva_ciudad, id_hoteles, temp_desc)][0];
                        if(temp < (*e1).dist){
                            (*e1).dist = temp;
